...
export function yearWindows(startISO, now = new Date()) {
  ...
  for (let year = startYear; year <= endYear; year++) {
    windows.push({
      ...
      to: year === endYear ? currentWindowEnd(now) : `${year}-12-31T23:59:59Z`,
    });
  }
  return windows;
}

// End of the current-year window. It must reach past "now" to catch today's
// contributions: GitHub buckets a contribution under the account's *local* day,
// but contributionsCollection filters the window by UTC timestamp. A commit made
// this evening in a behind-UTC timezone (e.g. US Pacific) carries a next-day UTC
// stamp, so ending the window at "now" drops today's contributions even though
// GitHub labels them today. Extend to the end of tomorrow (UTC), clamped to the
// year's end so the from/to span stays within the one year GitHub allows.
function currentWindowEnd(now) {
  ...
  return (tomorrowEnd < yearEnd ? tomorrowEnd : yearEnd).toISOString();
}
