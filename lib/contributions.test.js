...
import { combineCalendars, yearWindows, monthlyGrids, MAX_WEEK_ROWS } from "./contributions";
...
describe("yearWindows", () => {
  ...
  it("produces one calendar-year window per year from account creation to now", () => {
    expect(yearWindows("2024-03-15T00:00:00Z", now)).toEqual([
      ...
      { from: "2026-01-01T00:00:00Z", to: "2026-07-02T23:59:59.000Z" },
    ]);
  });

  it("extends the current-year window to end of tomorrow (UTC) to catch today's contributions", () => {
    // GitHub attributes a contribution to the account's local day but filters
    // the window by UTC timestamp, so today's late (behind-UTC) contributions
    // carry a next-day UTC stamp; ending at "now" would drop them.
    expect(yearWindows("2026-02-01T00:00:00Z", now)).toEqual([
      { from: "2026-01-01T00:00:00Z", to: "2026-07-02T23:59:59.000Z" },
    ]);
  });

  it("clamps the current-year window to the year's end (no spill into next year)", () => {
    const nye = new Date("2026-12-31T12:00:00Z");
    expect(yearWindows("2026-01-01T00:00:00Z", nye)).toEqual([
      { from: "2026-01-01T00:00:00Z", to: "2026-12-31T23:59:59.000Z" },
    ]);
  });
});
