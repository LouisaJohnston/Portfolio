...
// How many flowers or leaves are strewn along the ground.
export const GROUND_COUNT = 9;

// Where the i-th of them sits, as a percentage across. Strided like the motes
// so they don't march evenly across the ground, and derived from the index for
// the same reason: a re-render mustn't rearrange them. 4 and 9 share no factor,
// so the stride visits every position once.
export function groundSpot(i) {
  return Number((((i * 4) % GROUND_COUNT) * (100 / GROUND_COUNT) + 3).toFixed(2));
}
