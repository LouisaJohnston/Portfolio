...
describe("Navbar", () => {
  it("renders each social link in both the desktop and mobile menus", () => {
    ...
    expect(screen.getAllByRole("link", { name: /personal github/i })).toHaveLength(2);
    expect(screen.getAllByRole("link", { name: /linkedin/i })).toHaveLength(2);
  });

  it("lists LinkedIn before Personal GitHub in each menu", () => {
    render(<Navbar />);
    ...
    [desktop, mobile].forEach((menu) => {
      ...
      expect(labels).toEqual(["LinkedIn", "Personal GitHub"]);
    });
  });

  it("points both copies of a link at the same href", () => {
    render(<Navbar />);
    const github = screen.getAllByRole("link", { name: /personal github/i });
    github.forEach((link) =>
  ...
  it("opens external links safely in a new tab", () => {
    render(<Navbar />);
    const externalLinks = [
      ...screen.getAllByRole("link", { name: /personal github/i }),
